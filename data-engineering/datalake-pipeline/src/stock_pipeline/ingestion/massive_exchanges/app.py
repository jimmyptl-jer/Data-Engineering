"""
Massive Exchanges Ingestion Lambda — Entry Point.

Standalone Lambda that fetches exchange reference data from the
Massive (Polygon.io) REST API and lands it in the S3 Bronze layer.

Ingests:
    - get_exchanges(asset_class="stocks", locale="us")  → dataset=exchanges

Event Payload (optional):
    {
        "run_id": "batch_custom_001"   # optional — auto-generated if omitted
    }

SAM Handler:
    src.stock_pipeline.ingestion.massive_exchanges.app.lambda_handler
"""

import logging
import uuid
from datetime import datetime, timezone

from ...load import StockDataLoader
from ..massive_ingestion import MassiveIngestion
from ... import config


# ============================================================
# LOGGING
# ============================================================

logging.basicConfig(
    level=logging.INFO,
    format="%(levelname)s %(name)s %(message)s",
)

logger = logging.getLogger(__name__)


# ============================================================
# LAMBDA HANDLER
# ============================================================

def lambda_handler(event: dict, context) -> dict:
    """
    AWS Lambda entry point for Massive exchanges reference ingestion.

    Args:
        event (dict): Optional payload with keys:
            - run_id (str): Batch run identifier. Auto-generated UUID if omitted.
        context: AWS Lambda runtime context (unused).

    Returns:
        dict: statusCode, summary, and ingestion result.
    """

    execution_start_time = datetime.now(timezone.utc)

    logger.info(
        "[LAMBDA][MASSIVE_EXCHANGES_INGEST] Triggered at %s",
        execution_start_time.isoformat(),
    )

    # ----------------------------------------------------------
    # Parse event
    # ----------------------------------------------------------

    is_dict = isinstance(event, dict)
    run_id: str = (
        event.get("run_id", f"batch_{uuid.uuid4().hex[:8]}") if is_dict
        else f"batch_{uuid.uuid4().hex[:8]}"
    )

    logger.info(
        "[LAMBDA][MASSIVE_EXCHANGES_INGEST] run_id=%s",
        run_id,
    )

    # ----------------------------------------------------------
    # Initialise dependencies
    # ----------------------------------------------------------

    loader = StockDataLoader(
        aws_access_key_id=config.AWS_ACCESS_KEY_ID,
        aws_secret_access_key=config.AWS_SECRET_ACCESS_KEY,
    )

    ingestion = MassiveIngestion(
        loader=loader,
        bucket_name=config.S3_BUCKET_NAME,
    )

    # ----------------------------------------------------------
    # Ingest exchange reference data
    # ----------------------------------------------------------

    try:
        response = ingestion.ingest_exchanges(
            datasource="massive",
            execution_start_time=execution_start_time,
            run_id=run_id,
        )

        status = "SUCCESS" if response is not None else "FAILED"

        logger.info(
            "[LAMBDA][MASSIVE_EXCHANGES_INGEST] %s | run_id=%s",
            status,
            run_id,
        )

        return {
            "statusCode": 200,
            "run_id": run_id,
            "summary": {
                "source": "massive",
                "dataset": "exchanges",
                "status": status,
            },
            "response": str(response),
        }

    except Exception as e:
        logger.exception(
            "[LAMBDA][MASSIVE_EXCHANGES_INGEST] FAILED | run_id=%s | error=%s",
            run_id,
            e,
        )

        return {
            "statusCode": 500,
            "run_id": run_id,
            "summary": {
                "source": "massive",
                "dataset": "exchanges",
                "status": "FAILED",
                "error": str(e),
            },
        }


# ============================================================
# LOCAL EXECUTION
# ============================================================

if __name__ == "__main__":
    logger.info("[LOCAL] Running Massive exchanges ingestion locally.")
    result = lambda_handler({}, {})
    logger.info("[LOCAL] Done: %s", result)
