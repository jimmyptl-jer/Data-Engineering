    ...
    def ingest_stocks_list(
        ...
        try:
            ...
            try:
                ...
                logger.exception(
                    "[INGESTION][FINNHUB] Failed to fetch stock symbols | exchange=%s | run_id=%s",
                    exchange,
                    run_id,
                )
                ...
                if response:
                    logger.warning(
                        "[INGESTION][FINNHUB] Continuing with partial response | records=%d | run_id=%s",
                        len(response),
                        run_id,
                    )
                else:
            ...
            if not response:
                logger.warning(
                    "[INGESTION][FINNHUB] Empty API response | exchange=%s | run_id=%s",
                    exchange,
                    run_id,
                )
                return []
            ...
            logger.exception(
                "[INGESTION][FINNHUB] Ingestion failed | exchange=%s | run_id=%s",
                exchange,
                run_id,
            )
